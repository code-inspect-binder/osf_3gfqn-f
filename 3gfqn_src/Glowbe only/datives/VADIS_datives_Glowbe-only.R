#
# R code for analysis of DATIVES for the Inner Circle varieties only
# J Grafmiller
# 10/01/2019
#
...
#============================================================================
load("datives_descriptive.RData")
...
data <- subset(data, Corpus == "glowbe") %>%
  droplevels()
...
signif_line <- vadis_line1(glmer_list, path = "Glowbe-only/datives/line1.rds")
readRDS("Glowbe-only/datives/line1.rds", refhook = NULL)

#Individual components can be saved as .csv (or.txt) files like so.
write.csv(signif_line$signif.table,
          file = "Glowbe-only/datives/line1_significance_table.csv")
write.csv(as.matrix(signif_line$distance.matrix),
          file = "Glowbe-only/datives/line1_distance_matrix.csv")
write.csv(signif_line$similarity.scores,
          file = "Glowbe-only/datives/line1_similarity_scores.csv")
...
coef_line <- vadis_line2(glmer_list, path = "Glowbe-only/datives/line2.rds")
readRDS("Glowbe-only/datives/line2.rds", refhook = NULL)

#Individual components can be saved as .csv (or.txt) files like so.
write.csv(coef_line$coef.table,
          file = "Glowbe-only/datives/line2_coefficient_table.csv")
write.csv(as.matrix(coef_line$distance.matrix),
          file = "Glowbe-only/datives/line2_distance_matrix.csv")
write.csv(coef_line$similarity.scores,
          file = "Glowbe-only/datives/line2_similarity_scores.csv")
...
varimp_line <- vadis_line3(crf_list, path = "Glowbe-only/datives/line3.rds", conditional = FALSE)

#Individual components can be saved as .csv (or.txt) files like so.
write.csv(varimp_line$varimp.table,
          file = "Glowbe-only/datives/line3_varimp_table.csv")
write.csv(as.matrix(varimp_line$distance.matrix),
          file = "Glowbe-only/datives/line3_distance_matrix.csv")
write.csv(varimp_line$similarity.scores,
          file = "Glowbe-only/datives/line3_similarity_scores.csv")
...
write.csv(mean_sims, file = "Glowbe-only/datives/mean_scores_table.csv")
