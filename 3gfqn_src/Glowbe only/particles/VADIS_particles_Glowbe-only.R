...
f2 <- update(f1, .~. + (1|VerbPart_pruned))
...
signif_line <- vadis_line1(glmer_list, path = "Glowbe-only/particles/line1.rds")
readRDS("Glowbe-only/particles/line1.rds", refhook = NULL)

#Individual components can be saved as .csv (or.txt) files like so.
write.csv(signif_line$signif.table,
          file = "Glowbe-only/particles/line1_significance_table.csv")
write.csv(as.matrix(signif_line$distance.matrix),
          file = "Glowbe-only/particles/line1_distance_matrix.csv")
write.csv(signif_line$similarity.scores,
          file = "Glowbe-only/particles/line1_similarity_scores.csv")
...
coef_line <- vadis_line2(glmer_list, path = "Glowbe-only/particles/line2.rds")
readRDS("Glowbe-only/particles/line2.rds", refhook = NULL)

#Individual components can be saved as .csv (or.txt) files like so.
write.csv(coef_line$coef.table,
          file = "Glowbe-only/particles/line2_coefficient_table.csv")
write.csv(as.matrix(coef_line$distance.matrix),
          file = "Glowbe-only/particles/line2_distance_matrix.csv")
write.csv(coef_line$similarity.scores,
          file = "Glowbe-only/particles/line2_similarity_scores.csv")
...
varimp_line <- vadis_line3(crf_list, path = "Glowbe-only/particles/line3.rds", conditional = FALSE)

#Individual components can be saved as .csv (or.txt) files like so.
write.csv(varimp_line$varimp.table,
          file = "Glowbe-only/particles/line3_varimp_table.csv")
write.csv(as.matrix(varimp_line$distance.matrix),
          file = "Glowbe-only/particles/line3_distance_matrix.csv")
write.csv(varimp_line$similarity.scores,
          file = "Glowbe-only/particles/line3_similarity_scores.csv")
...
write.csv(mean_sims, file = "Glowbe-only/particles/mean_scores_table.csv")
